...
sf::RenderWindow window(sf::VideoMode(windowsize, windowsize), "Problem 2.1");
...
void drawShape(int x, int y, double radius) {
    ...
    window.draw(circle);
    int newx = (x - 7 * scale);
    //because of how the draw function in this library works my x axis needs to be inverse, function is effectively x2 = x1+7
    int newy = (y - 5 * scale);
    double newradius = sqrt(1.8 * (radius / scale)) * scale;
    //have to factor out the scale used to make the circle visible, function effectively newradius = sqrt(1.8*oldradius)
    if (newradius >= 1.9 * scale)
        drawShape(newx, newy, newradius);
    return;
}
